    ...
    <div className="flex h-[calc(100dvh-5.25rem)] max-h-[calc(100dvh-5.25rem)] min-h-0 flex-col gap-2 overflow-hidden text-sm">
      <header className="flex shrink-0 flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <p className="fs-eyebrow">Archive</p>
          <h1 className="mt-1 text-xl font-semibold tracking-tight text-text-primary">Year in Progress</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex items-center gap-1 rounded-xl border border-border bg-surface px-2 py-1">
            <button
              type="button"
      ...
      {!token && (
        <p className="fs-banner shrink-0">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-surface/70 p-4">
        {loading ? (
        ...
        <div className="mt-2 flex shrink-0 flex-wrap items-center justify-center gap-6 border-t border-border/80 pt-3 text-[10px] text-text-muted">
          <span className="flex items-center gap-2">
            <span className="inline-block rounded-full bg-gradient-to-br from-accent2 to-accent1 shadow-[0_0_8px_rgba(61,214,176,0.45)]" style={{ width: 12, height: 12 }} aria-hidden />
            Active day
          </span>
          <span className="flex items-center gap-2">
            <span
          ...
          <span className="flex items-center gap-2">
            <span
              className="inline-block rounded-full border border-dashed border-border bg-surface2/60"
              style={{ width: 12, height: 12 }}
              aria-hidden
            />
            Upcoming
...
function InlineStat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <span className="rounded-lg border border-border bg-surface px-2 py-1 tabular-nums">
...
function DayDot({ cell, size }: { cell: YearDayCell; size: number }) {
  ...
  if (cell.completed) {
    className +=
      'bg-gradient-to-br from-accent2 to-accent1 shadow-[0_0_12px_rgba(61,214,176,0.5)] ring-2 ring-accent2/30'
  } else if (cell.isFuture) {
    className += 'border border-dashed border-border bg-surface2/50'
  } else {
    className += 'border-2 border-border bg-surface/80'
  }
