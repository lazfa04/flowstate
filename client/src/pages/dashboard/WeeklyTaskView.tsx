...
function DayColumn({
  ...
  return (
    <article
      className={cn(
        'flex min-h-[320px] min-w-[10.5rem] flex-col rounded-2xl border p-5 backdrop-blur-sm',
        isToday ? 'border-accent1 ring-1 ring-accent1/40' : 'border-border/80',
      )}
      style={{ backgroundColor: tint }}
    >
      <header className="text-center">
        <p
          className={cn('font-semibold leading-tight', isToday ? 'text-accent1' : 'text-text-primary')}
          style={{ fontSize: '22px' }}
        >
          {weekday}
        </p>
                ...
                <li key={task.id} className="group rounded">
                  <div
                    className={cn(
                      ...
                      done ? 'text-text-primary' : 'border-l-2 bg-surface/80 text-text-primary',
                    )}
                    style={
                      done
      ...
      <button
        ...
        className="mt-2 flex w-full items-center justify-center gap-0.5 rounded-xl border border-dashed border-border bg-surface/50 py-1.5 text-xs text-accent5 hover:border-accent5/50 hover:bg-surface disabled:opacity-40"
        title="Add task"
      >
