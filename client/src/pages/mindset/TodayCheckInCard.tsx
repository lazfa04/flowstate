...
export function TodayCheckInCard({ token, todayKey, todayLog, loading, onSaved }: Props) {
  ...
  if (loading) {
    return (
      <div className="flex h-full min-h-0 gap-3 rounded-2xl border border-border bg-surface p-3">
  ...
  return (
    <div className="flex h-full min-h-0 gap-3 overflow-hidden rounded-2xl border border-border bg-surface px-4 py-3">
      ...
      <div className="flex shrink-0 items-center self-stretch border-l border-border/80 pl-3">
        {todayLog ? (
          <span className="whitespace-nowrap rounded-full bg-accent2/15 px-3 py-1.5 text-xs font-semibold text-accent2">
            Saved today ✓
          </span>
        ) : (
          <button
            ...
            onClick={() => void save()}
            className="fs-btn-primary whitespace-nowrap text-xs"
          >
